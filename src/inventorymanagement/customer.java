  ...
  public void tb_load(){
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        ...
        try {
            ...
            ResultSet rs = s.executeQuery("SELECT COUNT(*) as count FROM customer");
            if (rs.next() && rs.getInt("count") == 0) {
                // If table is empty, reset the auto-increment counter to 1
                s.executeUpdate("ALTER TABLE customer AUTO_INCREMENT = 1");
            } else {
                // Get the current maximum ID
                rs = s.executeQuery("SELECT MAX(cid) as max_id FROM customer");
                if (rs.next()) {
                    int maxId = rs.getInt("max_id");
                    // Reset auto-increment to the next available ID
                    s.executeUpdate("ALTER TABLE customer AUTO_INCREMENT = " + (maxId + 1));
                }
            }
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        ...
        try {
            ...
            if (rs.next()) {
                ...
                c_search_tb.setText(search); // Update the search text field
            } else {
                JOptionPane.showMessageDialog(this, "No customer found with ID: " + search,
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                ...
                // Get the current maximum ID
                ResultSet rs = s.executeQuery("SELECT MAX(cid) as max_id FROM customer");
                if (rs.next()) {
                    int maxId = rs.getInt("max_id");
                    // Reset auto-increment to the next available ID
                    s.executeUpdate("ALTER TABLE customer AUTO_INCREMENT = " + (maxId + 1));
